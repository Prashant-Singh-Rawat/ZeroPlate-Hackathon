...
import { UtensilsCrossed, Sun, Moon, Sparkles, QrCode } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar, activeView, onNavigateHome }) => {
  ...
  return (
    <>
      ...
      >
        <div className="w-full px-3 sm:px-6 lg:px-8 h-[60px] sm:h-[64px] flex items-center justify-between gap-2 sm:gap-3">
          {/* Brand + Mobile Toggle */}
          ...
          <div className="flex items-center gap-2">
            {/* Scan Mobile QR Code Button */}
            <button
              type="button"
              onClick={() => setShowQrModal(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-orange-200 dark:border-orange-900/60 bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/60 transition-all text-xs font-bold shadow-sm cursor-pointer"
              title="Scan QR Code on Mobile Phone"
            >
              <QrCode className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">Scan Mobile QR</span>
            </button>
