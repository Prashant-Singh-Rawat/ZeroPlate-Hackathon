...
import { UtensilsCrossed, Sun, Moon, Sparkles, Bell } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onToggleSidebar, activeView }) => {
  ...
  return (
    <header className="bg-white dark:bg-[#1E293B] border-b border-gray-100 dark:border-slate-800 sticky top-0 z-30 transition-colors duration-200"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[60px] flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="md:hidden p-2 text-gray-400 hover:text-gray-700 dark:hover:text-slate-100 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 transition-all"
          >
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 bg-gradient-to-br from-orange-500 to-amber-400 text-white rounded-xl shadow-md flex items-center justify-center">
              <UtensilsCrossed className="w-4.5 h-4.5" />
            </div>
            <div className="flex flex-col">
              <span className="text-base font-extrabold tracking-tight text-gray-900 dark:text-white leading-none">ZeroPlate</span>
              <span className="text-[10px] text-orange-500 font-semibold leading-none hidden sm:block">Share Food, Share Hope</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Portal badge */}
          <span className={`hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-extrabold border ${
            isDonor
          ...
          <div className="flex items-center gap-2 pl-2 border-l border-gray-100 dark:border-slate-700">
            ...
            <div className="hidden md:block text-left">
              <p className="text-xs font-extrabold text-gray-900 dark:text-slate-100 leading-none truncate max-w-[120px]">{user?.name}</p>
