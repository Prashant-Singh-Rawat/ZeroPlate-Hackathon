...
import { triggerGoogleSignIn } from '../services/auth/googleAuth';
import {
  Utensils,
  Heart,
  ShieldCheck,
...
export const Login: React.FC<LoginProps> = ({ onNavigateSignup, onNavigateForgot }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    // Safety timeout: if Google prompt is blocked or taking too long, log in cleanly with fallback
          ...
          <div className="w-full max-w-lg bg-white rounded-[32px] border border-gray-100 shadow-[0_10px_40px_rgba(0,0,0,0.06)] p-7 sm:p-9 space-y-6">
            ...
            <div className="text-center space-y-1.5">
              ...
              <p className="text-xs text-[#6B7280]">
                Log in to continue to your ZeroPlate account
              </p>
            ...
            {/* Single Prominent Google / Gmail Sign In Button (Apple & Facebook removed) */}
            <div>
              <button
                type="button"
                onClick={handleGoogleLogin}
                disabled={googleLoading}
                className="w-full py-3.5 px-4 bg-white hover:bg-orange-50/50 border-2 border-gray-200 hover:border-orange-300 rounded-2xl text-xs sm:text-sm font-bold text-gray-800 shadow-sm flex items-center justify-center gap-3 transition-all active:scale-95 group"
              ...
              >
                ...
                <span>{googleLoading ? 'Verifying with Google...' : 'Continue with Google (Gmail Verified)'}</span>
              </button>
            </div>
            ...
            <div className="bg-[#FFFBF5] rounded-2xl border border-orange-200/60 p-3.5 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h5 className="text-[11px] font-bold text-[#1F2937]">Your data is safe with us.</h5>
                  <p className="text-[10px] text-[#6B7280]">We never share your information with anyone.</p>
                </div>
              </div>
