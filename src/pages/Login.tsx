...
export const Login: React.FC<LoginProps> = ({ onNavigateSignup, onNavigateForgot }) => {
  ...
  // In-app Google Account Chooser modal fallback
  ...
  const handleGoogleLogin = () => {
    ...
    launchGoogleOAuthPopup(
      ...
      () => {
        // When popup is restricted or blocked, seamlessly open Google Account Chooser Modal
        setGoogleLoading(false);
  ...
  const handleCustomGoogleSubmit = (e: React.FormEvent) => {
    ...
    handleSelectGoogleAccount(customGoogleEmail, formattedName);
  };
    ...
    <div className="min-h-screen bg-[#FDFBF7] dark:bg-[#0B1120] flex flex-col justify-between text-[#1F2937] dark:text-slate-100 transition-colors duration-200">
      ...
      {showGoogleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-[#1E293B] rounded-3xl border border-gray-200 dark:border-slate-700 shadow-2xl w-full max-w-md p-6 sm:p-7 space-y-5 animate-scaleUp">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="text-base font-black text-gray-900 dark:text-white">Sign in with Google</h3>
              </div>
              <button
            ...
            <p className="text-xs text-gray-600 dark:text-slate-300">
              Choose an authorized Google Cloud account to continue to <strong>ZeroPlate ({selectedRole === 'donor' ? 'Food Donor' : 'NGO Manager'})</strong>:
            </p>

            <div className="space-y-2">
              {/* Primary Account: Aadarsh Sharma */}
              <button
                onClick={() => handleSelectGoogleAccount('aadarshsharma282@gmail.com', 'Aadarsh Sharma')}
                className="w-full p-3 bg-gray-50 dark:bg-slate-900/90 hover:bg-orange-50 dark:hover:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:border-orange-400 dark:hover:border-orange-500 rounded-2xl flex items-center justify-between transition-all group cursor-pointer"
              >
                <div className="flex items-center gap-3 text-left">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-orange-500 to-amber-500 text-white font-black flex items-center justify-center text-sm shadow-sm">
                    A
                  </div>
                  <div>
                    <span className="text-xs font-black text-gray-900 dark:text-white block group-hover:text-orange-600 dark:group-hover:text-orange-400">
                      Aadarsh Sharma
                    </span>
                    <span className="text-[11px] text-gray-500 dark:text-slate-400 block font-medium">
                      aadarshsharma282@gmail.com
                    </span>
                  </div>
                </div>
              ...
              {/* Demo Account based on role */}
              {selectedRole === 'donor' ? (
                <button
                  onClick={() => handleSelectGoogleAccount('donor@spicevilla.com', 'SpiceVilla Restaurant')}
                  className="w-full p-3 bg-gray-50 dark:bg-slate-900/90 hover:bg-orange-50 dark:hover:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:border-orange-400 dark:hover:border-orange-500 rounded-2xl flex items-center justify-between transition-all group cursor-pointer"
                >
                  <div className="flex items-center gap-3 text-left">
                    <div className="w-10 h-10 rounded-full bg-orange-100 dark:bg-orange-950 text-orange-600 font-black flex items-center justify-center text-sm">
                      S
                    </div>
                    <div>
                      <span className="text-xs font-black text-gray-900 dark:text-white block group-hover:text-orange-600 dark:group-hover:text-orange-400">
                        SpiceVilla Restaurant
                      </span>
                      <span className="text-[11px] text-gray-500 dark:text-slate-400 block font-medium">
                        donor@spicevilla.com
                      </span>
                    </div>
                  </div>
                ...
                <button
                  onClick={() => handleSelectGoogleAccount('ngo@hope.org', 'Hope Foundation')}
                  className="w-full p-3 bg-gray-50 dark:bg-slate-900/90 hover:bg-orange-50 dark:hover:bg-slate-800 border border-gray-200 dark:border-slate-700 hover:border-orange-400 dark:hover:border-orange-500 rounded-2xl flex items-center justify-between transition-all group cursor-pointer"
                >
                  <div className="flex items-center gap-3 text-left">
                    <div className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-600 font-black flex items-center justify-center text-sm">
                      H
                    </div>
                    <div>
                      <span className="text-xs font-black text-gray-900 dark:text-white block group-hover:text-orange-600 dark:group-hover:text-orange-400">
            ...
            <form onSubmit={handleCustomGoogleSubmit} className="pt-2 border-t border-gray-100 dark:border-slate-700 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl text-xs font-medium text-gray-900 dark:text-white focus:outline-none focus:border-orange-500"
                />
                <button
                  type="submit"
                  disabled={!customGoogleEmail}
                  className="px-3.5 py-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white font-bold text-xs rounded-xl shadow-sm transition-all flex items-center gap-1 cursor-pointer"
                >
                  <span>Sign In</span>
